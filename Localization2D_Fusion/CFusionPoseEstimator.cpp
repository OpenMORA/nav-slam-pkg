...
/* --------------------------------------------------------
				Ctor
   -------------------------------------------------------- */
CFusionPoseEstimator::CFusionPoseEstimator()
{
	reset();
}
...
/* --------------------------------------------------------
				reset
   -------------------------------------------------------- */
void CFusionPoseEstimator::reset()
{
...
void CFusionPoseEstimator::processUpdateNewPoseLocalization(
	const TPose2D &newPose,
	const CMatrixDouble33 &newPoseCov,
	const TPose2D refOdo,
	TTimeStamp cur_tim)
{
	CCriticalSectionLocker   lock(&m_cs);
	...
	if (m_last_odo_time!=INVALID_TIMESTAMP)
	{
		...
		//	dT,
		//	m_loc_odo_ref);
		m_loc_odo_ref = refOdo; 
	}
}

/** Updates the filter so the pose is tracked to the current time */
void CFusionPoseEstimator::processUpdateNewOdometry(
...
*/
bool CFusionPoseEstimator::getCurrentEstimate(
...
* \return true is the estimate can be trusted. False if the real observed data is too old.
*/
bool CFusionPoseEstimator::getLatestRobotPose(TPose2D &pose) const
{
	if (m_last_odo_time==INVALID_TIMESTAMP && m_last_loc_time==INVALID_TIMESTAMP )
		return false;
...
void CFusionPoseEstimator::extrapolateRobotPose(
	const TPose2D &p,
	const float v,
	const float w,
